          ...
          <>
               <Tabs defaultValue={tab} className="my-6 space-y-8">
                         <TabsList className="bg-transparent gap-2 justify-center w-full">
                              <TabsTrigger value={'top'} className="bg-muted data-[state=active]:border data-[state=active]:border-foreground" asChild>
                                   <Link href={`/explore${search !== undefined ? `?search=${search}` : ''}`}>
                                   Trending
                                   </Link>
                              </TabsTrigger>
                              <TabsTrigger value={'posts'} className="bg-muted data-[state=active]:border data-[state=active]:border-foreground" asChild>
                                   <Link href={`/explore?tab=posts${search !== undefined ? `&search=${search}` : ''}`}>
                                   Posts
                                   </Link>
                              </TabsTrigger>
                              <TabsTrigger value={'users'} className="bg-muted data-[state=active]:border data-[state=active]:border-foreground" asChild>
