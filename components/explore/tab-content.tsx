...
export default function ExploreComponent({users, posts, tags, session, search, tab, activeTab, searchParams, className}: {
     users: any, posts: any, tags: any, session: any, search: any, tab: any, activeTab: any, searchParams: any, className?: string
}) {
     return (
          ...
          {tags.length > 0 && (
               <Card>
                    ...
                    <CardContent>
                         <div className="flex flex-col divide-y">
                              {tags?.map((tag: any, index: number) => (
                                   <div className="flex items-center justify-between py-2.5" key={tag.id}>
                    ...
                    <CardFooter className="flex justify-end">
                         <Link href={`/explore?tab=tags${search !== undefined ? `&search=${search}` : ''}`} className="flex items-center text-sm text-muted-foreground font-medium">
                              More
                         </Link>
                    </CardFooter>
               </Card>
